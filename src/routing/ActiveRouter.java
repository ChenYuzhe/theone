...
public abstract class ActiveRouter extends MessageRouter {
    ...
    public Message messageTransferred(String id, DTNHost from) {
        ...
        //System.out.println("ActiveRouter:messageTransferred " + m.getMsgType() + " " + m.getId());
        /**
         //todo:按照请求包的内容检查自己的CachePointList中是否有这个content，如果有，发送reponse，如果没有，什么也不做；
         if (m.getMsgContent() != null && m.getMsgType().equals("request")) {
         if (getHost().checkNoContent(m.getMsgContent())) {
         //pass
         } else {
         m.setMsgType("responsed");
         getHost().weightUpdate(m.getMsgContent());
         Message res = new Message(this.getHost(), m.getFrom(),
         RESPONSE_PREFIX + m.getId(), m.getResponseSize(), getHost().getContent(m.getMsgContent()), "response");
         ...
         *  to zero.
         ...
         Message res = new Message(this.getHost(), m.getFrom(),
         RESPONSE_PREFIX + m.getId(), m.getResponseSize(), m.getMsgContent(), "response");
         this.createNewMessage(res);
         this.getMessage(RESPONSE_PREFIX + m.getId()).setRequest(m);
         }
         }
         */
        if (m.getMsgContent() != null && m.getMsgType().equals("response")) {

            getHost().weightUpdate(m.getMsgContent());

            if (getHost().checkNoContent(m.getMsgContent())) {//没有此数据
                getHost().addCachePoint(m.getMsgContent());
            }
        }
        getHost().hitratio();
