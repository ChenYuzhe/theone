...
public abstract class MessageRouter {
    ...
    public Message messageTransferred(String id, DTNHost from) {
        ...
        ///////////////////////////////////////////////////////////////////////////////
        if (aMessage.getMsgContent() != null && aMessage.getMsgType().equals("request")) {

            getHost().addTotalHit();
            getHost().weightUpdate(aMessage.getMsgContent());

            if (getHost().checkNoContent(aMessage.getMsgContent())) {
                //pass
            } else {
                getHost().addHit();
                //aMessage.setMsgType("responsed");
                Message res = new Message(this.getHost(), aMessage.getFrom(),
                        "R_" + aMessage.getId(), aMessage.getResponseSize(),
                        getHost().getContent(aMessage.getMsgContent()), "response");
                this.createNewMessage(res);
                this.getMessage("R_" + aMessage.getId()).setRequest(aMessage);
            }
        }
        ///////////////////////////////////////////////////////////
